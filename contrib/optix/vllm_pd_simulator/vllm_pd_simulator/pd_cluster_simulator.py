...
class PdClusterSimulator(SimulatorInterface):
    ...
    def _get_round_dir(self) -> Path:
        """Return this round's local script staging directory (per-cluster _gen_dir).

        Scripts are staged locally under _gen_dir (separate from the remote work dir
        _remote_dir) and then uploaded to every node's remote_dir during Stage 3; the
        upload step therefore no longer self-skips. The directory is (re)created
        idempotently on every call (surviving a mid-round _clear_remote_dirs) and is
        cleared each round by _reset_round_dir(); stop() deletes it via
        _cleanup_round_dir().
        """
        if self._round_tmp_dir is None:
            self._round_tmp_dir = Path(self._gen_dir)
            logger.info(f"[OPT] round dir (local staging): {self._round_tmp_dir}")
    ...
    def _clear_remote_dirs(self):
        ...
        upload always lands on a fresh directory; local staging (_gen_dir) is separate
        from remote_dir, so clearing remote dirs never affects the local scripts. Must
        run before Stage 0 (which repopulates the local round_dir) and must NOT run
        inside Stage 2/3, which depend on remote_dir being present.

        Docker mode: cleans BOTH the container-side dir (container_exec=True,
        ``docker exec <cid> sh -c "rm -rf ..."``) and the host-side dir
        (container_exec=False, direct ``rm -rf``) so host-side directories never
        survive across rounds.
        Bare-metal mode (no docker_container_id): the second call is equivalent
        to the first (SshRemote.run() ignores container_exec without a
        container id); the [ -d ] guard makes it a no-op when already removed.
        """
        seen: set = set()
        for node in self._all_nodes():
            ...
            try:
                ...
                # 20260915-01: 同一 [ -d ] 守卫清理命令分别以 container_exec=True/False 执行，
                # 同时清理容器侧与宿主侧目录，与 docstring 709-715 行声称一致。
                cmd = f"if [ -d {shlex.quote(self._remote_dir)} ]; then rm -rf {shlex.quote(self._remote_dir)}; fi"
                executor.run(
                    cmd,
                    container_exec=True,  # 容器侧: docker exec <cid> sh -c "..."
                    hide=True,
                    warn=True,
                ...
                executor.run(
                    cmd,
                    container_exec=False,  # 宿主侧: 该机直接执行
                    hide=True,
                    warn=True,
                    timeout=self._ssh_cmd_timeout,
                )
    ...
    def _gen_dir(self) -> str:
        """Per-cluster local script staging dir, separate from the remote work dir."""
        return f"{self._remote_tmp_dir}/vllm_pd_{self._cluster_id}_gen"
    ...
    def _upload_all_scripts(self):
        ...
        for node in self._all_nodes():
            ...
            try:
                executor = self._exec_for_node(node)
                # 20260831-04: 本地 staging (_gen_dir) 与远端目录分离后不再 self-skip，
                # 始终创建远端目录并上传（同机节点 localhost SCP 开销可忽略）。
                executor.run(
