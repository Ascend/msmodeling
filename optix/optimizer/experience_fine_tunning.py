...
from math import floor, isclose, isinf, isnan
...
class FineTune:
    def __init__(
        ...
        self.last_value = {}
        self.pd_initial_request_rate = None
    ...
    @staticmethod
    def is_field_adjustable(field: Optional[OptimizerConfigField]) -> bool:
        return field is not None and field.constant is None and not isclose(field.min, field.max)
    ...
    @classmethod
    def validate_pd_disaggregation_fields(cls, fields: Tuple[OptimizerConfigField, ...]) -> None:
        concurrency_field = cls.find_field(fields, CONCURRENCYS)
        request_rate_field = cls.find_field(fields, REQUESTRATES)
        ...
        if request_rate_field.constant is not None or isclose(request_rate_field.min, request_rate_field.max):
            raise ValueError(
                "PD disaggregation fine-tune requires a configurable REQUESTRATE range (min < max) "
                "to apply the measured QPS after the unlimited-rate probe."
            )
        if request_rate_field.max <= 0:
            raise ValueError("PD disaggregation fine-tune requires REQUESTRATE max > 0.")
    ...
    def prepare_pd_disaggregation_probe(self, params: np.ndarray):
        ...
        self.validate_pd_disaggregation_fields(simulate_run_info)
    ...
    def init_pd_disaggregation_request_rate(self, performance_index: PerformanceIndex) -> float:
        ...
        self.pd_initial_request_rate = request_rate
        return request_rate
    ...
    def handle_pd_disaggregation_concurrency(
        ...
        if concurrency_field is None or request_rate_field is None:
            raise ValueError("PD disaggregation fine-tune requires CONCURRENCY and REQUESTRATE.")
        if self.pd_initial_request_rate is None:
            raise ValueError("PD disaggregation initial request rate is not initialized.")
        if not request_rate_field.min <= self.pd_initial_request_rate <= request_rate_field.max:
            raise ValueError(
                f"Measured request rate {self.pd_initial_request_rate} is outside the configured "
                f"REQUESTRATE range [{request_rate_field.min}, {request_rate_field.max}]."
            )
        ...
        if performance_index.time_per_output_token > self.tpot_upper_bound:
            ...
            if self.pd_lower_concurrency is None:
                next_concurrency = max(current_concurrency / 2, concurrency_field.min)
            else:
            ...
            if self.pd_upper_concurrency is None:
                next_concurrency = min(current_concurrency * 2, concurrency_field.max)
            else:
                next_concurrency = (current_concurrency + self.pd_upper_concurrency) / 2

        if (
            concurrency_field.dtype == "int"
            and self.pd_lower_concurrency is not None
            and self.pd_upper_concurrency is not None
            and abs(self.pd_upper_concurrency - self.pd_lower_concurrency) <= 1
        ):
            # Concurrency cannot move further, but the caller may still tune request rate for TTFT.
            return False

        return self.set_field_value(concurrency_field, next_concurrency)
    ...
    def fine_tune_pd_disaggregation(self, params: np.ndarray, performance_index: PerformanceIndex):
        ...
        is_initial_request_rate = not self.last_value.get(REQUESTRATES)
        request_rate_updated = False
        if is_initial_request_rate:
            request_rate_updated = self.set_field_value(request_rate_field, self.pd_initial_request_rate)
        self.add_history(self.last_value, REQUESTRATES, request_rate_field.value)

        concurrency_updated = self.handle_pd_disaggregation_concurrency(simulate_run_info, performance_index)
        if is_initial_request_rate:
            if concurrency_updated or request_rate_updated:
                return simulate_run_info
            raise StopFineTune("Parameter value reached its boundary or did not change.")
        if concurrency_updated:
            return simulate_run_info

        self.check_config_and_performance(performance_index)
        self.direction_of_field_update(performance_index)
        if self.handle_request_rate(simulate_run_info, performance_index):
            return simulate_run_info
        raise StopFineTune("Parameter value reached its boundary or did not change.")
    ...
    def handle_concurrency(
        ...
        simulate_run_info: Tuple[OptimizerConfigField, ...],
        performance_index: PerformanceIndex,
        use_ttft_fallback: bool = False,
    ):
        was_updated_c = False
        signed_factor_c = None
        ttft_controls_concurrency = use_ttft_fallback and self.ttft_over_slo
        if ttft_controls_concurrency:
        ...
        if signed_factor_c:
            ...
            was_updated_c = self.update_field(
                ...
                force_min_change=self.tpot_over_slo or ttft_controls_concurrency,
            )
    ...
    def fine_tune_with_concurrency_and_request_rate(self, params: np.ndarray, performance_index: PerformanceIndex):
        ...
        if _concurrency_flag:
            was_updated_c = self.handle_concurrency(
                simulate_run_info,
                performance_index,
                use_ttft_fallback=not self.is_field_adjustable(_request_field),
            )
            if was_updated_c:
