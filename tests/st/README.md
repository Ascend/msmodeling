...
```text
tests/st/
├── regression.py               ← Main entry: total time regression + operator regression
...
├── model_config/               ← Video model configuration files
└── README.md                   ← This file
```
...
```json
{
  ...
  "model_id": "C:\\path\\to\\model_config\\Wan2.2-T2V-A14B-Diffusers",
  "seq_len": 128,
  "batch_size": 1,
  "height": 720,
  "width": 1280,
...
python -m pytest tests/st/regression.py -v --tb=short

# Filter by name
python -m pytest tests/st/regression.py -k "qwen3_30b" -v --tb=short
```
...
```bash
python -m pytest tests/st/regression.py -k "your_case_name" -v --tb=short
```
...
pytest tests/st/auto_baseline.py -v -s

# Filter by name
pytest tests/st/auto_baseline.py -k "qwen3-8B" -v -s
```
