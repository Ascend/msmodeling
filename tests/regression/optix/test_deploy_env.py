...
def _venv_parent_env(venv_root: Path) -> dict[str, str]:
    ...
    return {
        ...
        "PYTHONPATH": f"{site_packages}:/opt/custom/lib",
        "LD_LIBRARY_PATH": f"{venv_root / 'lib'}:/usr/lib",
...
def test_detect_in_virtualenv_by_prefix(monkeypatch):
    ...
    ctx = detect_runtime_context()
    assert ctx.in_virtualenv is True
    assert ctx.virtualenv_root == Path("/fake/venv")
...
def test_build_deploy_env_strips_conda_prefix_from_path(tmp_path):
...
def test_validate_mindie_succeeds_with_default_absolute_daemon(tmp_path, monkeypatch):
    default_daemon = "/usr/local/Ascend/mindie/latest/mindie-service/bin/mindieservice_daemon"
