...
"""Ownership: pso. Recommend first-run msmodeling optix search ranges.
...
#: 估算物理量（dtype 字节 / per-token KV / 权重总量 / KV 预算 / TP 整除候选）单一宿主：
#: estimation.py——与 experience_injector 共享，避免同一物理量双公式漂移
...
CONFIG_SKILL_SCRIPT = ".agents/skills/optix-assistant/scripts/config_writer.py"
...
def estimate_model_weight_gb(model: Dict[str, Any]) -> Tuple[float, str]:
    """估算权重全卡总量(GB)与来源文案。物理量公式单一宿主在
    estimation.estimate_weight_gb（十进制 1e9 口径）——本函数只保留
    model_weight_gb 直接提供 / 显式参数 / 结构兜底的来源语义。
    """
    if model.get("model_weight_gb"):
        return to_float(model["model_weight_gb"]), "provided model_weight_gb"
    note = (
        "estimated from num_parameters_billion"
        if model.get("num_parameters_billion")
        else ("estimated from num_parameters" if model.get("num_parameters") else "rough decoder-only estimate")
    )
    return estimate_weight_gb(model, dtype_bytes(model.get("torch_dtype"))), note
...
def choose_parallelism(context: Dict[str, Any], model_weight_gb: float) -> Dict[str, int]:
    ...
    # TP 整除候选来自 estimation.tp_candidates（物理量单一宿主），此处只叠加
    # 本脚本策略：<= per_node（节点内互联边界）
    valid_tp = [item for item in tp_candidates(world, heads) if item <= per_node]
    if not valid_tp:
        valid_tp = [1]

    reserve = min(8.0, card_mem * 0.12)
    tp = valid_tp[-1]
    for candidate in reversed(valid_tp):
...
def kv_capacity(context: Dict[str, Any], parallel: Dict[str, int], model_weight_gb: float) -> Dict[str, int]:
    ...
    reserve = min(8.0, card_mem * 0.12)
    # 预算公式单一宿主（estimation.kv_budget_gb），overhead 用本脚本的动态
    # reserve 策略（min(8, mem×0.12)）
    kv_mem_gb = kv_budget_gb(card_mem, model_weight_gb / parallel["tp"], gpu_util, reserve)

    # per-token KV 物理量单一宿主（estimation.per_token_kv_bytes）：显式
    # head_dim/kv_head_dim 优先、无则 hidden/n_heads 反推；GQA 分摊 / MLA·MQA 复制。
    # block 粒度并发上界是本脚本策略（与 paged 膨胀二选一的保守口径）。
    block = to_int(context.get("cache_block_size"), 128)
    per_token_bytes = per_token_kv_bytes(
        model,
        dtype_bytes(model.get("torch_dtype")),
        parallel["tp"],
    )["per_token_per_card_bytes"]
...
def target_field_token(value: Any) -> str:
    """CLI token，能被 config_writer.coerce_deep 还原：bool/int/float/str 原样，
    list/dict（enum/factories/… 的 dtype_param）压成紧凑 JSON。
    """
...
def command_for_config_skill(item: Dict[str, Any]) -> Optional[str]:
    """生成写 [[<engine>.target_field]] 块的 config_writer --target-field 命令。

    只对引擎段（mindie/vllm）发写命令：config_writer --target-field 的 engine 只接受引擎段。
    ais_bench 段字段（CONCURRENCY/REQUESTRATE 等 scheduler env，真实 config 中按运行引擎落在
    [[vllm|mindie.target_field]]）保留在 handoff `target_fields` JSON 里，由 agent 按引擎合并，
    不在这里硬编码成单个引擎 —— 与 build_toml 的 "kept in JSON handoff" 一致。
    """
    if not item.get("target_field", True):
        return None
    section = item["section"]
    if section not in {"mindie", "vllm"}:
        return None
    ...
    kv = [
        f"engine={section}",
        f"name={item['name']}",
        f"config_position={item.get('config_position', 'env')}",
        f"dtype={item['dtype']}",
    ]
    if "value" in item:
        kv.append(f"value={target_field_token(item['value'])}")
    if "min" in item:
        kv.append(f"min={item['min']}")
    if "max" in item:
        kv.append(f"max={item['max']}")
    if "dtype_param" in item:
        kv.append(f"dtype_param={target_field_token(item['dtype_param'])}")

    args = ["python", CONFIG_SKILL_SCRIPT, "--target-field", *kv]
...
def build_config_skill_handoff(engine: str, recs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "version": 1,
        "consumer_skill": "optix-assistant",
...
def build_toml(engine: str, benchmark: str, recs: List[Dict[str, Any]], world_size: int) -> str:
    ...
    sections = [
        "# Generated by optix-assistant (PSO branch). Review before applying.",
