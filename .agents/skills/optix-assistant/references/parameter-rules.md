...
- 默认 benchmark：`ais_bench`（昇腾生态最常用；**仍须询问用户确认**，选择准则见 `benchmark.md`）。
...
用户未指定 benchmark 时输出 `ais_bench` 字段（默认值；**流程上仍须询问确认**，见 `benchmark.md`）。
...
- 配置写入应用（optix-assistant）：看 `config_skill_handoff`。
