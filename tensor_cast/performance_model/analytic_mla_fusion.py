...
def rescale_analytic_mla_bundle(events: Sequence) -> None:
    ...
    members = []
    for event in events:
        result = event.perf_results.get("analytic")
        if result is None:
            continue
        # Cached signatures share Results; mutate an event-private copy.
        result = PerformanceModel.Result(
            execution_time_s=result.execution_time_s,
            statistics=_copy_stats(result.statistics),
        )
        event.perf_results["analytic"] = result
        components = BoundAnalyzer.components(result)
        compute = components.mma_ops_time_s + components.gp_ops_time_s
        memory = components.memory_time_s
        members.append((mla_family_role(event.op_invoke_info), result, compute, memory))
    if len(members) < 2:
        return
    ...
    other_m = sum(memory for role, _r, _c, memory in members if role not in {"prefill_projection", "decode_projection"})
